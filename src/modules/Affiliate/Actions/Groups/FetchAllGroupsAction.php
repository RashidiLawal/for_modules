 ...
 *
 * Handles fetching and filtering affiliate groups.
 */
class FetchAllGroupsAction extends GroupAction
{
    #[OA\Get(
    ...
    protected function action(): Response
    {
        try {
            ...
            return $this->respondWithData([
                'status' => true,
                'message' => trans("Affiliate::messages.groups_fetched"),
                'groups' => $groups
            ]);
        } catch (\Exception $e) {
            $this->logger->error("Failed to fetch affiliate groups: " . $e->getMessage());

            return $this->respondWithData([
                'status' => false,
                'error' => $e->getMessage(),
                'message' => trans("Affiliate::messages.groups_fetch_failed"),
            ], 500);
        }
    }
}
